...
The description follows the guidelines of \citet{Nordlie09_e1000456}.
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{tabular}{
  ...
  \hline
  thalamic neurons $\mathcal{T}$\newline (thalamo-cortical inputs)  & realizations of Poisson point process & $N_\text{TC}$ \\
  \hline 
\end{tabular}
%%%%%%%%%%%%%%%%%%%
...
\begin{tabular}{
  ...
  \textbf{Description} & dynamics of membrane potential $V_{i}(t)$ and spiking activity $s_i(t)$ of neuron $i\in x$ for $x\in\{\mathcal{E}_{23},\ldots,\mathcal{I}_{6}\}$:
                         ...
                         \item emission of $k$th ($k=1,2,\ldots$) spike of neuron $i$ at time $t_{i}^{k}$ if
                           \begin{equation}
                             V_{i}\left(t_{i}^{k}\right)\geq{}V_\text{th}  
                           \end{equation}
                           with spike threshold $V_\text{th}$
			     ...
			     \label{eq:membrane_potential}
                             \begin{aligned}
                               ...
                               \Bigl[V_\text{rest}-V_i(t)\Bigr]+\RM I_i(t) + \RM I_{i,\mathcal{C}_y}
                             \end{aligned}
                           \end{equation}
                           with membrane time constant $\tauM$, membrane resistance $\RM$, resting potential $V_\text{rest}$, total synaptic input current $I_i(t)$ (see ``Synapses''), and constant background current $I_{i,\mathcal{C}_y}=I_{\mathcal{C}_y}$ for $i\in{}y$ (see ``Cortico-cortical inputs'')
...
\begin{equation*}
	I_{\mathcal{C}_x} = K_{\mathcal{C}_x} \cdot I_\text{CC}
  \quad (\forall x\in\{\mathcal{E}_{23},\ldots,\mathcal{I}_{6}\})\, ,
\end{equation*}
	with cortico-cortical in-degree $K_{\mathcal{C}_x}$, and mean current
  \begin{equation*}
    I_\text{CC} = \bar{I}\cdot\tauS\cdot\nu_\text{CC}
  \end{equation*}
  generated by single cortico-cortical input spike train with rate $\nu_\text{CC}$, convolved with an exponential kernel with amplitude $\bar{I}$ and time constant $\tauS$ (see remark \ref{remark:cortico_cortical_dc})
  \\
%%%%%%%%%%%%%%%%%%%%%
  \hline 
  \multicolumn{2}{|>{\color{black}\columncolor{lightgray}}c|}{\textbf{Thalamic neurons (thalamo-cortical inputs)}}\\
  \hline
  ...
  \begin{equation}
    ...
    \begin{cases}
      0 & t<t_\text{TC,start}\\
       \nu_\text{TC} & t_\text{TC,start} \le t < t_\text{TC,stop}\\
      0 & t\ge{}t_\text{TC,stop}\\
    \end{cases}
  \end{equation}\\
  \hline
%%  
...
\begin{tabular}{
  ...
  \begin{itemize}
	...
	\item solution of \eqref{eq:synaptic_current} for $\displaystyle f_{ij}(t)=\hat{I}_{ij}s_{j}(t)$ and $I_{ij}(t=0)=0$:
		\begin{equation}
			\text{PSC}_{ij}(t)=\hat{I}_{ij} \exp(-t/\tauS) V_\text{th}(t)
		\end{equation}
	...
	\item solution of \eqref{eq:membrane_potential} for $I_i(t)=\text{PSC}_{ij}(t)$, $V_i(t=0)=0$, and $V_\text{rest}=0$:
                           \begin{equation}
                             \text{PSP}_{ij}(t)=
  ...
  \begin{equation}
      d_{ij} = \begin{cases} 
	      \text{max}(d_\text{min},z_{yx}), & \forall i,\ \forall j \in x\in\{\mathcal{E}_{23},\ldots,\mathcal{I}_{6},\mathcal{T}\} \\
                 ...
                 \bar{d}_\text{CC}, & \forall i,\ \forall j \in x=\mathcal{C}_y \\                 
      \end{cases}
  \end{equation}
  with
  \begin{equation}
...
\begin{tabular}{
  ...
  \hline
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
\multicolumn{3}{|>{\color{black}\columncolor{lightgray}}c|}{\textbf{Neurons}}\\
\hline 
...
\hline  
$V_\text{th}$ & $-50\mV$ & spike threshold \\
\hline
...
\hline  
$\nu_\text{CC}$ & $8\sps$ & rate of a single cortico-cortical input \\
\hline
\drvd{$I_\text{CC}$} & \drvd{$\bar{I}_\text{CC}\tauS\nu_\text{CC} = 0.3 \pA$} & mean current generated by a single cortico-cortical input\\
\hline
...
%%%%%%%%%%%%%%%%%%%%%
\hline
\multicolumn{3}{|>{\color{black}\columncolor{lightlightgray}}c|}{\textbf{Thalamic neurons (thalamo-cortical inputs)}}\\
\hline    
$\nu_\text{TC}$ & $120\sps$ & rate of thalamic neurons \\
\hline
$t_{\text{TC,start}}$ & $700\ms$ & start time of thalamo-cortical input\\
\hline
$\Delta_\text{TC}$ & $10\ms$ & duration of thalamo-cortical input \\
\hline
\drvd{$t_{\text{TC,stop}}$} & \drvd{$t_{\text{TC,start}}+\Delta_\text{TC} = 710\ms$} & stop time of thalamic input\\
\hline
\end{tabular}
...
\begin{tabular}{
    ...
    |}
\hline
\multicolumn{3}{|>{\color{black}\columncolor{lightgray}}c|}{\textbf{Synapses}}\\
\hline 
\textbf{Name} & \textbf{Value} & \textbf{Description}\\
\hline 
...
$\bar{d}_{yx}$ &  & mean spike transmission delays\\
              & $1.5\ms$ & $x\in\left\{ \mathcal{E}_{23}, \mathcal{E}_{4}, \mathcal{E}_{5}, \mathcal{E}_{6},\mathcal{T}\right\} $,
$y\in\left\{\mathcal{E}_{23},\mathcal{E}_{4},\mathcal{E}_{5},\mathcal{E}_{6},\mathcal{I}_{23},\mathcal{I}_{4},\mathcal{I}_{5},\mathcal{I}_{6}\right\}$
 ...
 & $0.75\ms$ & $x\in\left\{ \mathcal{I}_{23}, \mathcal{I}_{4}, \mathcal{I}_{5}, \mathcal{I}_{6}\right\}$, 
               $y\in\left\{\mathcal{E}_{23},\mathcal{E}_{4},\mathcal{E}_{5},\mathcal{E}_{6},\mathcal{I}_{23},\mathcal{I}_{4},\mathcal{I}_{5},\mathcal{I}_{6}\right\}$ \\
  % \hline
  % $\bar{d}_\text{E}$ & $1.5\ms$ & mean spike transmission delay for excitatory connections\\
  % \hline
  % $\bar{d}_\text{I}$ & $0.75\ms$ & mean spike transmission delay for inhibitory connections\\
  %\hline
  %$\bar{d}_\text{T}$ & $1.5\ms$ & spike transmission delay for thalamic inputs\\
  \hline
  $\text{CV}_\text{d}$ & $0.5$ & coefficient of variation of delay distribution\\
  %\hline
  %$\text{CV}_\text{d,T}$ & $0.5$ & coefficient of variation of thalamic delay distribution\\
 \hline
  $\sigma_{\text{d},yx}$ & $\text{CV}_\text{d}\cdot\bar{d}_{yx}$ & standard deviation of spike transmission delays\\
  \hline
  $d_\text{min}$      & $0.1\ms$      & minimal spike transmission delay\\
  \hline
  $\bar{d}_\text{CC}$ & $1.5\ms$ & spike transmission delay for cortico-cortical inputs\\  
\hline
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
...
\item linear, inhomogeneous dynamics of synaptic input currents and (subthreshold) membrane potential for neuron $i \in y\in\{\mathcal{E}_{23},\ldots,\mathcal{I}_{6}\}$ (cf.~eqs.\,\eqref{eq:membrane_potential} and \eqref{eq:synaptic_current}):
  \begin{equation}
    ...
    \begin{aligned}
      ...
      \dot{V}_{i} + \frac{1}{\tauM} \Bigl[ V_{i}-V_\text{rest}\Bigr] - \frac{\RM}{\tauM} I_{i} &= 0
    \end{aligned}
  \end{equation}
  with
  \begin{equation}
...
\item rescale membrane potential $v_i(t) = V_i(t)-V_\text{rest}$ and total
...
\item \label{remark:network_scaling}
  ...
  \begin{equation}
    ...
    I_i(t)=\sum_{x\in\{\mathcal{E}_{23},\ldots,\mathcal{I}_{6}\}} \sum_{j\in{}x} (s_j* \text{PSC}_{ij})(t-d_{ij}) + I_{\text{CC},i}
  \end{equation}
  ...
  %%
  \par  
  Under the assumption that the spike trains $s_j(t)$ can be described by stationary Poisson point processes with rates $\nu_j$, the mean and the variance of the total synaptic input current \cref{eq:total_synaptic_input_current} read\footnote{Here, we have absorbed the constant cortico-cortical input current $I_{\text{CC},i}$ into the sum across all presynaptic sources.}
  \begin{equation}
...
\item \label{remark:dc_scaling_constraints}
  ...
  \begin{equation}
    \label{eq:rheobase_current}
     I_{\text{rh},y} = \frac{V_\text{th} - V_\text{rest}}{\RM} ,
  \end{equation}
...
In \citep{Senk26_012001}, the population specific initial conditions are referred to as \emph{amended} initial conditions.
  %%% 
\end{enumerate}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\end{appendices}
...
\bibitem[Nordlie et al., 2009]{Nordlie09_e1000456}
   Nordlie, E., Gewaltig, M.O., Plesser, H.E. (2009).
  \newblock Towards reproducible descriptions of neuronal network models.
  \newblock {\em PLoS Computational Biology} 5(8):e1000456.
  \newblock \url{https://doi.org/10.1371/journal.pcbi.1000456}

\bibitem[Potjans \& Diesmann, 2014]{Potjans14}
...
\bibitem[Senk et al., 2022]{Senk22_e1010086}
  ...
  \bibitem[Senk et al., 2026]{Senk26_012001}
  Senk, J., Kurth, A., Furber, S., Gemmeke, T., Golosio, B., Heittmann, A., Knight, J. C., M\"uller, E., Noll, T., Nowotny, T., Peraza Coppola, G., Peres, L., Rhodes, O., Rowley, A., Schemmel, J., Stadtmann, T., Tetzlaff, T., Tiddia, G., van Albada, S. J., Villamar, J., \& Diesmann, M. (2026). 
  \newblock Constructive community race: full-density spiking neural network model drives neuromorphic computing.
  \newblock {\em Neuromorphic Computing and Engineering} 6(1):012001.
  \newblock \url{http://dx.doi.org/10.1088/2634-4386/ae379a}  
